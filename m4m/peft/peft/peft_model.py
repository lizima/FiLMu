...
from m4m.llama.model import MusicEncoder
# from m4m.llama.model_3 import MusicEncoder
# print('CAUTION: import model_3')
...
class PeftModelForCausalLM(PeftModel):
    ...
    def __init__(self, model: torch.nn.Module,
                 peft_config: PeftConfig,
                 adapter_name: str = "default",
                 embeded_token_path: str = "weights/embeded_tokens.npy", ) -> None:
        super().__init__(model, peft_config, adapter_name)
        self.base_model_prepare_inputs_for_generation = self.base_model.prepare_inputs_for_generation
        self.music_encoder = MusicEncoder(embeded_token_path, device=model.device)
    ...
    def load_music_encoder(self, model_folder):
        path = os.path.join(model_folder, "music_encoder.pth")
        self.music_encoder.load_state_dict(torch.load(path))

    def save_music_encoder(
            self,
            save_directory: str,
    ) -> None:
        self.music_encoder.save_weights(save_directory)
    ...
    def save_pretrained(
        ...
        super(PeftModelForCausalLM, self).save_pretrained(
        ...
        self.save_music_encoder(save_directory=save_directory)

    def generate(self, *args, **kwargs):
        print("peft_model generate")
        peft_config = self.active_peft_config
